...
async function sendMediaMessage(uniqueID, filePath) {
  try {
...
async function sendSingleShloka(user_data) {
  if (user_data[2]) {
...
function sendShlok(nextshlok = false, uniqueID) {
    try {
      let count = 0;
      fs.readFile("data.json", "utf8", async (err, content) => {
        ...
        if(nextshlok == false) {
          ...
          sendSingleShloka(data[uniqueID])
        }
      });
    } catch (e) {
      console.error(e);
